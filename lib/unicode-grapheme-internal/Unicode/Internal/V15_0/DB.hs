...
import Unicode.Internal.Version (UnicodeVersion (UnicodeVersion_15_0))
...
mkUnicodeDatabaseIO =
  MkUnicodeDatabase <$> readUnicodeDataIO asserts UnicodeVersion_15_0
  where
    ...
    asserts =
      MkPropertiesI
        { derivedCoreProperties =
            MkDerivedCorePropertiesI
              { indicConjunctBreakConsonant = 0,
                indicConjunctBreakExtend = 0,
                indicConjunctBreakLinker = 0
              },
          emojiData =
            MkEmojiDataI
