module Unicode.Grapheme.Generator.DB.V17_0
  ( generateModule,
  )
where
...
import Unicode.Grapheme.Generator.Version (UnicodeVersion (UnicodeVersion_17_0))

generateModule :: Maybe OsPath -> Maybe OsPath -> IO ()
generateModule mDataDir mDestDir = do
  derivedTxt <- DerivedCoreProperty.generateData mDataDir (911, 2217, 20) vers
  eastAsianWidth <- DerivedEastAsianWidth.generateData mDataDir (104, 126606) vers
  emojiTxt <- EmojiData.generateData mDataDir (1219, 2848) vers
    ...
    gbpAsserts =
      HMap.fromList
        [ (GraphemeClusterBreak_Prepend, 27),
          ...
          (GraphemeClusterBreak_Extend, 2237),
          (GraphemeClusterBreak_Regional_Indicator, 26),
          (GraphemeClusterBreak_SpacingMark, 381),
